...
{
	if (!m_bCached) fprintf (fOut,"%d.%d.%d.%d\t%s\n",m_nIp&255,(m_nIp>>8)&255,(m_nIp>>16)&255,m_nIp>>24,m_sHostname);
	if (more) more->Print(fOut);
	if (less) less->Print(fOut);
}
...
char * CNameResolution::Resolve (const char * sIp)
{
	...
	*bt_next = new CNameNode;
	if ((*bt_next)==NULL) throw CError ("not enough memory");
	(*bt_next)->m_nIp = nIp;
...
{
	if (WSAStartup (MAKEWORD(2,0),&m_wsadata)) throw CError ("unable to init winsock");
	m_oCache = NULL;
	...
	if (fCache) {
		...
		while (!feof(fCache)) {
			if (fgets(sLine,MAX_FILENAME_LENGTH,fCache)==NULL) {
				if (ferror(fCache)) throw CError ("error reading cache file");
			} else {
				sHostname = NULL;
...
CNameResolution::~CNameResolution()
{
	...
	if (m_sCacheFile) {
		...
		if (!fCache) throw CError ("unable to open cache file for appending");
		if (m_oCache) m_oCache->Print (fCache);
		fclose (fCache);
	...
	if (m_nHostnamesTotal>0) {
		...
		printf ("Resolving time: %.2fs\n",float(m_nTimeWaiting)/1000.0f);
	}

	if (m_sCacheFile) delete m_sCacheFile;
	if (m_oCache) delete m_oCache;
}
